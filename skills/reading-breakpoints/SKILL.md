...
2. **关键字触发创建新文件**：当用户说「新的一天」或「继续」作为阅读分享的开头时，无论日期是否一致，都立即创建新文件（用于连续对话跨天时主动分段）
3. 将摘抄或感慨格式化为 `## Slide N: 标题` 条目。摘抄和「开源之道」感慨是**同等且独立的素材**——可以单独成 slide，也可以合并在一张 slide 中：
   - 仅有摘抄 → 直接成 slide
   - 仅有感慨 → 直接成 slide
...
**注意**：NotebookLM 每次最多生成 15 张 slide。当 slide 数量超过 15 张时，必须将文件分割为多个部分，文件名后加 `-part-1`、`-part-2` 等序号，每部分不超过 15 张 slide。分割后的文件封面 slide 需要保留系统提示，并更新标题标明 Part N。
